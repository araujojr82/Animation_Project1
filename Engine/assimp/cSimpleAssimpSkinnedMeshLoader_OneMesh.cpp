								              ...
								              std::vector<glm::mat4> &Offsets)
{
	...
	this->ReadNodeHeirarchy(AnimationTime, animationName, this->pScene->mRootNode, Identity, pAnimation );
...
//	}
	...
	if ( pNodeAnim )
	{
		...
		std::string nodeName = NodeName.data;
		
		glm::mat4 TranslationM;
		//if( nodeName == "ValveBiped_Bip01_Pelvis" )
		//{
		//	if( this->pPreviousAnimationName == "" || animationName != this->pPreviousAnimationName )
		//	{
		//		this->CalcGLMInterpolatedPosition( 0, pNodeAnim, this->pelvisStartingPosition );
		//	}
		//	
		//	glm::vec3 pos;
		//	this->CalcGLMInterpolatedPosition( AnimationTime, pNodeAnim, pos );

		//	pos.x = this->pelvisStartingPosition.x;
		//	pos.z = this->pelvisStartingPosition.z;

		//	TranslationM = glm::translate( glm::mat4( 1.0f ), pos );

		//	this->pPreviousAnimationName = animationName;
		//}
		//else
		//{
...
cMesh* cSimpleAssimpSkinnedMesh::CreateMeshObjectFromCurrentModel( unsigned int meshIndex /*=0*/)
{
	...
	for ( int vertIndex = 0; vertIndex != pTheMesh->numberOfVertices; vertIndex++ )
	{
		...
		if ( pAIMesh->GetNumColorChannels() > 0 )
		{
			...
			pCurVert->r = this->pScene->mMeshes[0]->mColors[0]->r;
			pCurVert->g = this->pScene->mMeshes[0]->mColors[0]->g;
			pCurVert->b = this->pScene->mMeshes[0]->mColors[0]->b;
			pCurVert->a = this->pScene->mMeshes[0]->mColors[0]->a;
		}
		else
		{
